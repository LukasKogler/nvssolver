...
# mesh_file = "planemesh.vol.gz"
# pickle_file_sol = "stokes_sol.pickle"
# nref = 0
# maxh = 1000
# # ----- Code to exec --- 
# __generated_code_to_execute = ""
# # --- GENERATED PART END --- 
...
nu = 0.001
Re = Uref * Lref / nu
mesh = gen_ref_mesh(geo=None, mesh_file=mesh_file, nref=nref, load=True, comm=mpi_world, maxh=maxh)
uin = CoefficientFunction( (-Uref,0,0) )
flow_settings = FlowOptions(geom = None, mesh = mesh, nu = nu, inlet = "front|top|bottom|right|left", outlet = "back", wall_slip = "",
                            wall_noslip = "plane", uin = uin, symmetric = False, vol_force = None)
...
def doit():
    ...
    try: # not ideal solution, but whatever
        ...
        import pickle
        pickle.dump((u_vis, p_vis), open(pickle_file_sol.replace(".pickle", "_" + str(mpi_world.rank) + ".pickle"), "wb"))
    except:
        pass
